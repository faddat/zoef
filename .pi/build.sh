...
# docker buildx build --tag zoef --file .pi/Dockerfile --platform linux/arm64 --cache-from zoef:cache --cache-to zoef:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar zoef
...
sudo bash -c "echo zoef > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/zoef.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/zoef.img
